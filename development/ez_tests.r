...
eta2partial <- SSeffect / (SSeffect + SSerror)

to_return$ANOVA$pes = to_return$ANOVA$SSn/(to_return$ANOVA$SSn+to_return$ANOVA$SSd)


eta2general <- (SSeffect - (dfeffect)(MSerror)) / (MSerror + SStotal)


ges.test <- (112 - 2*18.333)/(18.33 + 610)
ges.test2 <- 112/(112+330)


to_return$ANOVA$ges = to_return$ANOVA$SSn/(to_return$ANOVA$SSn+sum(unique(to_return$ANOVA$SSd))+obs_SSn1-obs_SSn2)

eta2general <- SSeffect/(SSeffect+sum(SSerror))+
...
summary(aov(Recall ~ (Task*Valence)+Error(Subject/(Task*Valence)), data))
ezANOVA(wid = Subject, within = .(Valence, Task), dv = Recall, data = data, detailed = TRUE)




ezANOVA(data, wid = Subject, between = .(Dosage, Gender), observed = Gender, within = .(Task, Valence), dv = Recall, detailed = TRUE, type = 2)
